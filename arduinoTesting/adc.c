...
//For Arduino Usage. If time, research how to do ifdef ARDUINO, then takeVtpMeasurement has the following implementation, ifndef, then look below
float measureVtp()
{
  int v_tp_adc = analogRead(V_TP_PIN);
  float v_tp = v_tp_adc * (v_ref_int / 1024);
  return v_tp;
}
